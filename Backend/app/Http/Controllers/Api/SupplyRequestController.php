...
class SupplyRequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $supplyRequest->load(['user', 'product', 'branch']);
            $staffName = $supplyRequest->user?->full_name ?? "Staff #{$supplyRequest->user_id}";
            $productName = $supplyRequest->product?->name ?? "Product #{$supplyRequest->product_id}";
            $branchName = $supplyRequest->branch?->name ?? 'Undefined Branch';

            Notification::create([
                'type' => 'stock_request',
                'message' => "{$staffName} requested {$supplyRequest->quantity} stock of {$productName} for {$branchName}",
                'data' => [
                    'supply_request_id' => $supplyRequest->id,
                    'user_id' => $supplyRequest->user_id,
                    'user_name' => $staffName,
                    'product_id' => $supplyRequest->product_id,
