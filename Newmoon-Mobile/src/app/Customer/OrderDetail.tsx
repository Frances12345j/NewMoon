    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      >
        ...
        {(order.status === 'picked_up' || order.status === 'out_for_delivery') && (
          <>
            <TouchableOpacity
              className="bg-yellow-400 rounded-xl py-3 items-center mb-3"
            ...
            <TouchableOpacity
              className="bg-amber-50 border border-amber-200 rounded-xl py-3 items-center mb-4"
              onPress={() => router.push(`/Customer/orderChat?orderId=${order.id}`)}
              activeOpacity={0.7}
            >
              <View className="flex-row items-center">
                <Ionicons name="chatbubble-ellipses-outline" size={18} color="#D97706" />
                <Text className="text-amber-700 font-semibold ml-2">Chat with Rider</Text>
              </View>
            </TouchableOpacity>
          </>
        )}
